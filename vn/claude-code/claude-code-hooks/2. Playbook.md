...
| **JSON validation failed** | Shell startup scripts (`.zshrc`) output text | Wrap output trong `if [[ $- == *i* ]]` check |
...
| **Shell output trong `.zshrc`** | Polute hook JSON parsing | Wrap trong `[[ $- == *i* ]]` |
